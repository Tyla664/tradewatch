...
function formatPrice(p, id) {
  ...
  // Forex pairs: full pip precision
  if (id && id.includes('/') && !id.startsWith('XAU') && !id.startsWith('XAG')) {
    // JPY pairs trade to 3 decimal places (pip = 0.01, sub-pip = 0.001)
    if (id.includes('JPY')) return p.toFixed(3);
    // All other forex: 5 decimal places (pip = 0.0001, sub-pip = 0.00001)
    return p.toFixed(5);
  }
...
function refreshSelectedAssetPanel() {
  ...
  if (price) {
    // Round to appropriate precision — matches formatPrice logic
    const _isJPY    = asset.id?.includes('JPY');
    const _isForex  = asset.id?.includes('/') && !asset.id?.startsWith('XAU') && !asset.id?.startsWith('XAG');
    const decimals  = _isForex ? (_isJPY ? 3 : 5) : price >= 1000 ? 2 : price >= 1 ? 4 : 6;
...
function ensureLWChart() {
  ...
  // Determine price format for this asset — full pip precision for forex
  const _chartAsset = asset;
  const _isJPYChart = _chartAsset?.id?.includes('JPY');
  const _isForexChart = _chartAsset?.id?.includes('/') &&
    !_chartAsset?.id?.startsWith('XAU') && !_chartAsset?.id?.startsWith('XAG');
  const _chartPriceFormat = _isForexChart
    ? { type: 'price', precision: _isJPYChart ? 3 : 5, minMove: _isJPYChart ? 0.001 : 0.00001 }
